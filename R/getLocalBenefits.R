...
#' }
#' @name getLocalBenefits
NULL

#' @rdname getLocalBenefits
#' @export
getLocalBenefits <- function(x) {
  ...
  if(inherits(x, "Solution")){
    ...
    for(it in 1:length(x$data)){
      ...
      if(cont_aux == 0){
        benefit_solution <- getLocalBenefits(x$data[[it]])
        colnames(benefit_solution)[3] <- paste0("sol",it)
        cont_aux = 1
      }
      else{
        aux <- getLocalBenefits(x$data[[it]])
        benefit_solution[, it + 2] <- aux[,3]
        colnames(benefit_solution)[it + 2] <- paste0("sol",it)
      }
    }
