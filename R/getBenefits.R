...
#' @rdname getBenefits
#' @export
getBenefits <- function(x) {
  # assert argument is valid
  assertthat::assert_that(inherits(x, c("Solution", "ConservationProblem", "Portafolio")))
  ...
  if(inherits(x, "Solution")){
    ...
    if(getStatusCode(x) %in% !c(1,3)){
      ...
      benefits <- rcpp_stats_benefit(x$data$pu,
                                     ...
                                     x$data$sensitivity,
                                     0)
      benefits$benefit_n_sol <- NA
      benefits$benefit_rec_sol <- NA
    }
    return(benefits)
  }
    ...
    for(it in 1:length(x$data)){
      ...
      if(cont_aux == 0){
        benefit_solution <- getBenefits(x$data[[it]])
        colnames(benefit_solution)[7] <- paste0("benefit_n_sol",it)
        colnames(benefit_solution)[8] <- paste0("benefit_rec_sol",it)
        cont_aux = 1
      }
      else{
        aux <- getBenefits(x$data[[it]])
        benefit_solution[, (it*2 - 1) + 6] <- aux[,7]
        benefit_solution[, (it*2) + 6] <- aux[,8]
        colnames(benefit_solution)[(it*2 - 1) + 6] <- paste0("benefit_n_sol",it)
        colnames(benefit_solution)[(it*2) + 6] <- paste0("benefit_rec_sol",it)
      }
    }
