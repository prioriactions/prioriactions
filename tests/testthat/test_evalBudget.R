test_that("evaluate methods with random values", {
  ...
  budget_values = sample(0:10, 3, replace = FALSE)

  port <- suppressWarnings(evalBudget(pu = pu_sim,
  ...
  expect_equal(port$getNames(), paste0("Budget", base::round(budget_values, 3)))
})
